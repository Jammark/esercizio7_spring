...
public class PrenotazioniRunner implements CommandLineRunner {
	...
	public void run(String... args) throws Exception {
		...
		List<Postazione> lista = new ArrayList<Postazione>();
		try {
			List<Postazione> lista1 = poDao.findByEdificio(edificio1);
			if (lista1.isEmpty()) {
				Postazione p1 = (Postazione) ctx.getBean("getPostazione");
			...
			List<Postazione> lista2 = poDao.findByEdificio(edificio2);
			if (lista2.isEmpty()) {
				Postazione p2 = (Postazione) ctx.getBean("getPostazione");
			...
			ricerca = poDao.findByCittà(edificio1.getCittà(), TipoPostazione.SALA_RIUNIONI);
			ricerca.forEach(e -> log.info(TipoPostazione.SALA_RIUNIONI.name() + "" + e));

			User u1 = (User) ctx.getBean("getUtente");
			Postazione p = lista.get(0);
			LocalDate data = LocalDate.now().plusDays(3);
			if (prDao.isOccupata(p, data)) {
				log.info("Postazione " + p + " prenotata in data " + data);
			} else {
				Prenotazione pr = new Prenotazione(data, u1, p);
				prDao.save(pr);
				boolean check = prDao.isOccupata(p, data);
				log.info("Postazione " + p + (check ? " già occupata " : " non occupata ") + "in data " + data);
			}

			Scanner sc = new Scanner(System.in);
			log.info("Selezionare una postazione.");
			lista.forEach(post -> log.info(1 + lista.indexOf(post) + " per postazione in edificio "
					+ post.getEdificio().getNome() + " tipo " + post.getTipo()));
			int num = selectNumero(sc, lista.size());
			Postazione post = lista.get(--num);
			log.info("Inserire numero di giorni a partire dalla data corrente, max 100");
			int numData = selectNumero(sc, 100);
			data = LocalDate.now().plusDays(numData);

			if (prDao.isOccupata(post, data)) {
				log.info("Postazione " + post);
				log.info(" occupata in data " + data);
			} else {
				Prenotazione pr = new Prenotazione(data, u1, post);
				prDao.save(pr);
				boolean check = prDao.isOccupata(post, data);
				log.info("Postazione " + p);
				log.info("" + (check ? " prenotata " : " non prenotata ") + "per data " + data);
			}

			log.info("Selezionare un utente");
			List<User> lu = uDao.findAll();
			lu.forEach(u -> log.info(1 + lu.indexOf(u) + " per " + u.getNomeCompleto()));
			int numU = selectNumero(sc, lu.size());
			User user = lu.get(--numU);
			log.info("Inserire numero di giorni a partire dalla data corrente, max 100");
			numData = selectNumero(sc, 100);
			data = LocalDate.now().plusDays(numData);
			boolean free = prDao.isPuòPrenotare(user, data);
			if (free) {
				log.info("L'utente " + user.getNomeCompleto() + " è libero di prenotare in data " + data);
			} else {
				log.info("L'utente " + user.getNomeCompleto() + " non può prenotare in data " + data);
			}
